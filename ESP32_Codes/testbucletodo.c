...
void loop() {
  Serial.println(i);
  float arraytiempo[20];
  float arrayoutpotencia[20];
  float arraydutycycle[20];
  float dutyfinal;
  float arrayouttension[20];
  struct timeval tv;
  ...
  if (i > 20) {
    ...
    Serial.println(i);
    Serial.println("funcion send to excel"); int count = 0;
    for (count = 0; count < 20; count = count + 1) {
      Serial.println("Entro a for");
      Serial.println(count);
      String urlFinal = "https://script.google.com/macros/s/" + GOOGLE_SCRIPT_ID + "/exec?" + "tiempo=" + String(arraytiempo[count]) +"&outpotencia=" + String(arrayoutpotencia[count]) + "&dutycycle=" + String(arraydutycycle[count])  + "&outtension=" + String(arrayouttension[count]) + "&dutycyclefinal=" + String(dutyfinal);
      Serial.println("define url");
      HTTPClient http;
      Serial.println("Inicia cliente");
      http.begin(urlFinal.c_str());
      Serial.println("escribe el url, creo");
      http.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);
      Serial.println("Redirect");
      http.end();
      Serial.println("Termina HTTP");
    }
    count = 0;
  }
  delay(100);
